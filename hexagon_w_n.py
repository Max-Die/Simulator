...
""" Does the same as hexagon.py but wihtout noise (so the shape is perfectly sharp) """
def draw_hexagon_w_n(img, difficulty, size_min, size_max, pixel_processing, rand):
  heigth = img.shape[0]
  width = img.shape[1]
  ...
  # get coordinates of pixel belonging to the hexagon
  row, col = draw.polygon((y1, y2, y3, y4, y5, y6), (x1, x2, x3, x4, x5, x6))
  points = []
  k = 0
  ...
  xmin = min(bb[0][0], bb[1][0], bb[2][0], bb[3][0], bb[4][0], bb[5][0])
  ymin = min(bb[0][1], bb[1][1], bb[2][1], bb[3][1], bb[4][1], bb[5][1])
  xmax = max(bb[0][0], bb[1][0], bb[2][0], bb[3][0], bb[4][0], bb[5][0])
  ymax = max(bb[0][1], bb[1][1], bb[2][1], bb[3][1], bb[4][1], bb[5][1])
  bb = []
  bb.append(xmin)
  bb.append(ymin)
  bb.append(xmax)
