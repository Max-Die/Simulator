...
""" This file draws a rectangle on an image. """
def draw_rectangle(img, interruption, missing_fragments, sharpness_of_edges, difficulty, size_min, size_max, pixel_processing, rand):
  heigth = img.shape[0]
  width = img.shape[1]
  ...
  # get coordinates of pixel belonging to the rectangle
  row, col = draw.rectangle(start = (ys, xs), end = (ye, xe))
  points = []
  x = rand.randint(xmin + 4, xmax - 4)
  d = rand.randint(10, 40)
  k = 0
  # rotate the figure and if wanted delete some fragments from the rectangle
