...
""" This file draws an triangle on an image. """
def draw_triangle(img, interruption, missing_fragments, sharpness_of_edges, difficulty, size_min, size_max, pixel_processing, rand):
  heigth = img.shape[0]
  width = img.shape[1]
  ...
  # get coordinates of pixel belonging to the triangle
  row, col = draw.polygon((y1, y2, y3), (x1, x2, x3))
  points = []
  x = rand.randint(xmin, xmax)
  d = rand.randint(20, 40)
  # rotate the figure and if wanted delete some fragments from the triangle
  ...
  xmin = min(bb[0][0], bb[1][0], bb[2][0])
  ymin = min(bb[0][1], bb[1][1], bb[2][1])
  xmax = max(bb[0][0], bb[1][0], bb[2][0])
  ymax = max(bb[0][1], bb[1][1], bb[2][1])
  bb = []
  bb.append(xmin)
  bb.append(ymin)
  bb.append(xmax)
