...
""" This file draws an triangle contur on an image. """
def draw_triangle_contur(img, interruption, sharpness_of_edges, difficulty, size_min, size_max, pixel_processing, rand):
  heigth = img.shape[0]
  width = img.shape[1]
  ...
  # get coordinates of pixel belonging to the triangle contur 
  points = []
    ...
    for j in range(len(row)):
      ...
      if interruption:
          if x_old > x - x1 and x_old < x - x1 + d:
            continue
      points.append([round(x_old * math.cos(angle) - y_old * math.sin(angle)) + rand.randint(-sharp, sharp) + x1, round(x_old * math.sin(angle) + y_old * math.cos(angle)) + rand.randint(-sharp, sharp) + y1])
