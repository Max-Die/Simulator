...
""" Does the same as heptagon.py but wihtout noise (so the shape is perfectly sharp) """
def draw_heptagon_w_n(img, difficulty, size_min, size_max, pixel_processing, rand):
  heigth = img.shape[0]
  width = img.shape[1]
  s_min = round(size_min / 2.25)
  s_max = round(size_max / 2.25)
  x1 = rand.randint(200, width - 200)
  y1 = rand.randint(200, heigth - 200)

  x2 = rand.randint(x1 + s_min, x1 + s_max)
  y2 = rand.randint(y1 - s_min, y1 + s_min)
    
  x3 = rand.randint(x2 + s_min, x2 + s_min + 5)
  ...
  y5 = rand.randint(y4 - s_min, y4 + s_min)
      
  x6 = rand.randint(x5 - s_max, x5 - s_min)
  y6 = rand.randint(y5 - s_max, y5 - s_min)
    
  x7 = rand.randint(x6 + s_min, x6 + s_min + 5)
  y7 = rand.randint(y6 - s_max, y6 - s_min)
  angle = rand.randint(0, 180)

  bb = []

  xmin = min(x1, x2, x3, x4, x5, x6, x7)
  xmax = max(x1, x2, x3, x4, x5, x6, x7)
  ...
  # get coordinates of pixel belonging to the heptagon
  row, col = draw.polygon((y1, y2, y3, y4, y5, y6, y7), (x1, x2, x3, x4, x5, x6, x7))
  points = []
  k = 0
  ...
  xmin = min(bb[0][0], bb[1][0], bb[2][0], bb[3][0], bb[4][0], bb[5][0], bb[6][0])
  ymin = min(bb[0][1], bb[1][1], bb[2][1], bb[3][1], bb[4][1], bb[5][1], bb[6][1])
  xmax = max(bb[0][0], bb[1][0], bb[2][0], bb[3][0], bb[4][0], bb[5][0], bb[6][0])
  ymax = max(bb[0][1], bb[1][1], bb[2][1], bb[3][1], bb[4][1], bb[5][1], bb[6][1])
  bb = []
  bb.append(xmin)
  bb.append(ymin)
  bb.append(xmax)
